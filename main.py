...
while True:
    ...
    if user_choice == False:
        ...
        if view_stat_type == False:
            ...
            incomplete_tasks = filter_tasks(tasks=all_tasks)
            for index, task in enumerate(incomplete_tasks[:3]):                          #⬅️ (⚠️ need testing) Accesses & prints ur next 3 incomplete tasks. Starts @ 0, counts upwards until 3, excluding 3.
                print(format_task_line(task=task, task_index=index))
        elif view_stat_type == 3:
            completed_tasks = filter_tasks(tasks=all_tasks, done=True)
            for index, task in enumerate(completed_tasks[-5:]):                          #⬅️ (⚠️ need testing) Accesses & prints ur last 5 completed tasks. Counts down backwads -5 from the end of list.
                print(format_task_line(task=task, task_index=index))
